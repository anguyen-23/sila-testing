# Generated by sila2.code_generator; sila2.__version__: 0.14.0
# Implementation by learning project
from __future__ import annotations

import logging
from typing import TYPE_CHECKING

from sila2.server import MetadataDict

from ..generated.camerastreamprovider import (
    CameraNotFound,
    CameraStreamProviderBase,
    GetLatestFrame_Responses,
    StartStream_Responses,
    StopStream_Responses,
    StreamError,
)

if TYPE_CHECKING:
    from ..server import Server

log = logging.getLogger(__name__)


def _make_placeholder_jpeg() -> bytes:
    """Generate a minimal valid 1x1 gray JPEG for simulation mode."""
    return bytes([
        ,
    ])


class CameraStreamProviderImpl(CameraStreamProviderBase):
    """Camera streaming feature using OpenCV.

    In simulation mode, returns a placeholder JPEG.
    In real mode, captures from IP cameras (HTTP/RTSP) or USB webcams.
    """

    def __init__(self, parent_server: Server) -> None:
        super().__init__(parent_server=parent_server)
        self._simulation_mode: bool = True
        self._sim_streaming: bool = False
        self._capture = None  # Lazy import of CameraCapture

    def _get_capture(self):
        """Lazy-load the CameraCapture to avoid OpenCV import errors when not installed."""
        if self._capture is None:
            from ..camera_capture import CameraCapture
            self._capture = CameraCapture()
        return self._capture

    def get_IsStreaming(self, *, metadata: MetadataDict) -> bool:
        if self._simulation_mode:
            return self._sim_streaming
        return self._get_capture().is_streaming

    def StartStream(self, Source: str, *, metadata: MetadataDict) -> StartStream_Responses:
        log.info("StartStream: source='%s', simulation=%s", Source, self._simulation_mode)

        if self._simulation_mode:
            self._sim_streaming = True
            return StartStream_Responses()

        try:
            self._get_capture().start(Source)
        except RuntimeError as e:
            if "Cannot open" in str(e):
                raise CameraNotFound(str(e))
            raise StreamError(str(e))
        except Exception as e:
            raise StreamError(str(e))
        return StartStream_Responses()

    def StopStream(self, *, metadata: MetadataDict) -> StopStream_Responses:
        log.info("StopStream")

        if self._simulation_mode:
            self._sim_streaming = False
            return StopStream_Responses()

        self._get_capture().stop()
        return StopStream_Responses()

    def GetLatestFrame(self, *, metadata: MetadataDict) -> GetLatestFrame_Responses:
        if self._simulation_mode:
            if not self._sim_streaming:
                raise StreamError("Not streaming. Call StartStream first.")
            return GetLatestFrame_Responses(Frame=_make_placeholder_jpeg())

        capture = self._get_capture()
        frame = capture.latest_frame
        if frame is None:
            raise StreamError("No frame available. Ensure stream is running and camera is accessible.")
        return GetLatestFrame_Responses(Frame=frame)
