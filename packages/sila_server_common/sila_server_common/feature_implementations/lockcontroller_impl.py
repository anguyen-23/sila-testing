"""LockController implementation — canonical, shared across all servers.

Uses the sila2 built-in LockController types directly (no per-server generated code needed).
This is the version with the "unlocked guard fix" — only requires LockIdentifier
metadata when the server is actually locked.
"""
...
class LockControllerImpl(LockControllerBase):
    ...
    def get_calls_affected_by_LockIdentifier(self) -> List[Union[Feature, Command, Property, FullyQualifiedIdentifier]]:
        # Only require LockIdentifier metadata when the server is actually locked.
        if not self.is_locked:
            return []
        return [
            feature
...
class LockControllerInterceptor(MetadataInterceptor):
    ...
    def intercept(self, parameters: Any, metadata: MetadataDict, target_call: FullyQualifiedIdentifier) -> None:
        if not self.lockcontroller_impl.is_locked:
            # Server is not locked, allow the call without requiring metadata
            return
