...
def load_screen_config(server_package_or_dir: str | Path) -> dict | None:
    """Load screen_capture.json from a server's root directory.
    ...
    """
    config_path = _screen_config_path(server_package_or_dir)
    if config_path is None or not config_path.exists():
        return None
    try:
        return json.loads(config_path.read_text(encoding="utf-8"))
    except Exception as e:
        logger.warning("Failed to load screen capture config from %s: %s", config_path, e)
        return None


def save_screen_config(server_package_or_dir: str | Path, config: dict) -> None:
    """Save screen_capture.json to a server's root directory."""
    config_path = _screen_config_path(server_package_or_dir)
    if config_path is None:
        logger.warning("Cannot determine config path for screen capture")
        return
    try:
        config_path.write_text(json.dumps(config, indent=4) + "\n", encoding="utf-8")
        logger.info("Saved screen capture config to %s", config_path)
    except Exception as e:
        logger.warning("Failed to save screen capture config to %s: %s", config_path, e)


def _screen_config_path(server_package_or_dir: str | Path) -> Path | None:
    """Resolve the path to screen_capture.json for a server."""
    if isinstance(server_package_or_dir, Path):
        return server_package_or_dir / "screen_capture.json"
    import importlib
    try:
        pkg = importlib.import_module(server_package_or_dir)
        return Path(pkg.__file__).parent.parent / "screen_capture.json"
