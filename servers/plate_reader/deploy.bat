@echo off
:: Plate Reader SiLA 2 Server - One-click deploy script
::
:: Usage:
...
echo.
echo ============================================
echo   Plate Reader SiLA 2 Server - Deploy
echo ============================================
echo.
...
echo.
echo [2/3] Installing plate_reader and dependencies...
...
netsh advfirewall firewall show rule name="SiLA Plate Reader" >nul 2>&1
if errorlevel 1 (
    echo      Adding firewall rule for port 50054...
    netsh advfirewall firewall add rule name="SiLA Plate Reader" dir=in action=allow protocol=tcp localport=50054 >nul 2>&1
    if errorlevel 1 (
...
if "%INSTALL_ONLY%"=="1" (
    ...
    echo   "%VENV_DIR%\Scripts\python.exe" -m plate_reader --insecure -a 0.0.0.0 -p 50054 --verbose
    pause
    exit /b 0
)
...
if "%MODE%"=="simulation" (
    ...
    echo ============================================
    echo.
    "%VENV_DIR%\Scripts\python.exe" -m plate_reader --insecure -a 0.0.0.0 -p 50054 --verbose
) else (
