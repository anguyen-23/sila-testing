@echo off
:: Quick-start the Plate Reader SiLA 2 server (after deploy.bat has been run)
::
:: Usage:
...
echo Plate Reader SiLA 2 Server - port %PORT%

if "%MODE%"=="real" (
    echo Mode: REAL
    "%VENV_DIR%\Scripts\python.exe" -m plate_reader --insecure -a 0.0.0.0 -p %PORT% --verbose
) else (
