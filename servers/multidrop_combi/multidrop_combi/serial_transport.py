...
class MultidropSerialTransport:
    ...
    def _send_and_receive(self, cmd: str, timeout: float | None = None) -> list[str]:
        ...
        try:
            ...
            logger.debug("TX: %r", cmd, extra={"category": "serial", "direction": "tx", "serial_cmd": cmd})
            self._serial.write(f"{cmd}\r".encode("ascii"))
            self._serial.flush()
            ...
            while True:
                ...
                if not raw:
                    raise ConnectionError(f"Timeout reading response for {cmd_code}")
                line = raw.decode("ascii", errors="replace").strip()
                logger.debug("RX: %r", line, extra={"category": "serial", "direction": "rx"})
                if not line:
                    continue
                lines.append(line)
            ...
            if status_code != STATUS_OK:
                desc = ERROR_DESCRIPTIONS.get(status_code, "Unknown error")
                logger.error("Command %s failed (status %d). All RX lines: %s",
