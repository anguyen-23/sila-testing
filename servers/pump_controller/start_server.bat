@echo off
:: Quick-start the Pump Controller SiLA 2 server (after deploy.bat has been run)
::
:: Usage:
...
echo Pump Controller SiLA 2 Server - port %PORT%

if "%MODE%"=="real" (
    echo Mode: REAL
    "%VENV_DIR%\Scripts\python.exe" -m pump_controller --insecure -a 0.0.0.0 -p %PORT% --verbose
) else (
