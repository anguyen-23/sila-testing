@echo off
:: Pump Controller SiLA 2 Server - One-click deploy script
::
:: Usage:
...
echo.
echo ============================================
echo   Pump Controller SiLA 2 Server - Deploy
echo ============================================
echo.
...
echo.
echo [2/3] Installing pump_controller and dependencies...
...
netsh advfirewall firewall show rule name="SiLA Pump Controller" >nul 2>&1
if errorlevel 1 (
    echo      Adding firewall rule for port 50053...
    netsh advfirewall firewall add rule name="SiLA Pump Controller" dir=in action=allow protocol=tcp localport=50053 >nul 2>&1
    if errorlevel 1 (
...
if "%INSTALL_ONLY%"=="1" (
    ...
    echo   "%VENV_DIR%\Scripts\python.exe" -m pump_controller --insecure -a 0.0.0.0 -p 50053 --verbose
    pause
    exit /b 0
)
...
if "%MODE%"=="simulation" (
    ...
    echo ============================================
    echo.
    "%VENV_DIR%\Scripts\python.exe" -m pump_controller --insecure -a 0.0.0.0 -p 50053 --verbose
) else (
